  ...
  return (
    <div className="space-y-5 animate-fade-in" id="report-content">
      <div className="flex items-center justify-between flex-wrap gap-3 print:hidden">
        <div>
          <h2 className="font-bold text-xl text-slate-900">Relatórios</h2>
      ...
      <Card>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {[
            { label:'Receita Total',      value: data.incomeTotal,      color:'text-primary-dark' },
            { label:'Despesas Previstas', value: data.expensesPlanned,  color:'text-danger-dark'  },
            { label:'Despesas Pagas',     value: data.expensesPaid,     color:'text-danger-dark'  },
            { label:'Saldo Atual',        value: data.currentBalance,   color: data.currentBalance   >= 0 ? 'text-primary-dark' : 'text-danger-dark' },
            { label:'Saldo Projetado',    value: data.projectedBalance, color: data.projectedBalance >= 0 ? 'text-primary-dark' : 'text-danger-dark' },
            { label:'Dívida Ativa Total', value: data.totalActiveDebt,  color:'text-warning-dark' },
          ].map((item) => (
            <div key={item.label} className="bg-subtle rounded-2xl p-4">
        ...
        {data.commitment && (
          <div className="mt-4 pt-4 border-t border-border">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-semibold text-slate-700">Comprometimento da Renda</p>
      ...
      {health && (
        <Card>
          ...
          <div className="flex items-center gap-5 mb-5">
            ...
            <div>
              <p className="text-3xl font-bold text-slate-900">{health.score} <span className="text-base text-muted font-normal">/ 100 pontos</span></p>
          ...
          {health.breakdown && (
            <div className="space-y-3">
              {Object.entries(health.breakdown).map(([key, f]) => (
                <div key={key} className="flex items-center gap-3 p-3 bg-subtle rounded-xl">
                  <div className="h-2 w-2 rounded-full shrink-0" style={{ backgroundColor: f.points >= f.max * 0.6 ? '#10B981' : f.points > 0 ? '#F59E0B' : '#EF4444' }} />
                  <p className="text-sm text-slate-700 flex-1">{f.reason}</p>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <span className="text-sm font-mono font-bold text-slate-700 w-10 text-right">{f.points}/{f.max}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      {(data.alerts ?? []).length > 0 && (
        <Card>
          ...
          <div className="space-y-2">
            {data.alerts.map((a) => (
              <div key={a.id} className={`flex items-start gap-3 p-3.5 rounded-xl border ${a.resolvedAt ? 'bg-subtle border-border opacity-60' : a.severity === 'critical' ? 'bg-danger-subtle border-danger/20' : a.severity === 'warning' ? 'bg-warning-subtle border-warning/20' : 'bg-info-subtle border-info/20'}`}>
          ...
          <div className="space-y-3">
            {data.recommendations.map((r, i) => (
              <div key={i} className="border border-border rounded-2xl p-4">
                ...
                {r.calculation && (
                  <p className="text-xs text-muted bg-subtle rounded-xl p-2 mt-2 font-mono">{r.calculation}</p>
                )}
              </div>
            ))}
          </div>
      ...
      {(data.goals ?? []).length > 0 && (
        <Card padding={false}>
          <div className="px-5 py-4 border-b border-border">
            <h3 className="font-semibold text-slate-900">Relatório de Metas Ativas</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border/60">
                {data.goals.map((g) => (
                  <tr key={g.id} className="hover:bg-subtle/40 transition-colors">
                    <td className="table-cell font-semibold">{g.name}</td>
                    <td className="table-cell font-mono tabular-nums">{formatCurrency(g.targetValue)}</td>
                    <td className="table-cell font-mono tabular-nums text-primary-dark font-bold">{formatCurrency(g.progress)}</td>
                    <td className="table-cell">
                      <div className="flex items-center gap-2">
                        <div className="flex-1 h-2 bg-subtle rounded-full overflow-hidden min-w-16">
                          <div className="h-full bg-primary rounded-full" style={{ width: `${Math.min(g.percentage, 100)}%` }} />
                        </div>
                        <span className="text-xs font-mono font-bold text-slate-700 shrink-0">{Math.round(g.percentage)}%</span>
                      </div>
                    </td>
      ...
      {(data.cards ?? []).length > 0 && (
        <Card padding={false}>
          <div className="px-5 py-4 border-b border-border">
            <h3 className="font-semibold text-slate-900">Relatório de Cartões</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-subtle/60"><tr>
                ...
                {data.cards.map((c) => {
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-subtle/40 transition-colors">
                      ...
                      <td className="table-cell font-mono tabular-nums text-danger-dark">{formatCurrency(c.usedLimit)}</td>
                      <td className="table-cell font-mono tabular-nums text-primary-dark">{formatCurrency(c.availableLimit)}</td>
                      <td className="table-cell">
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-2 bg-subtle rounded-full overflow-hidden min-w-16">
