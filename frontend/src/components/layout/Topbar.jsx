...
import {
  IconMenu, IconBell, IconSun, IconMoon, IconChevronL, IconChevronR,
} from '../icons';
...
export function Topbar({ title }) {
  ...
  const loadAlerts = useCallback(async () => {
    ...
    try {
      ...
      // Falha silenciosa — alertas não são críticos para UX
    }
  }, [selectedId]);
  ...
  return (
    <header className="sticky top-0 z-10 glass border-b border-border/60 dark:border-white/[0.06] h-16 flex items-center gap-3 px-4 sm:px-6">
      ...
      <h1 className="font-semibold text-slate-900 dark:text-zinc-50 text-lg truncate flex-1">{title}</h1>

      {month && (
        <div id="month-switcher" className="flex items-center gap-1 bg-subtle dark:bg-white/5 border border-border dark:border-white/10 rounded-xl px-2 py-1.5">
