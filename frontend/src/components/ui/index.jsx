...
export function Card({ children, className = '', padding = true, hover = false }) {
  return (
    <div className={`bg-white dark:bg-panel-dark rounded-2xl border border-border dark:border-white/[0.06] shadow-card dark:shadow-premium-dark theme-transition ${padding ? 'p-5' : ''} ${hover ? 'transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 cursor-pointer' : ''} ${className}`}>
      {children}
    </div>
  );
}
