...
async function getOrCreateInvoice(card, refMonth, refYear) {
  ...
  const month = await monthsService.getOrCreateMonth(card.userId, refMonth, refYear);
...
async function createCardPurchase(userId, payload) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    for (let i = 1; i <= installmentsCount; i += 1) {
      ...
      const value = i === installmentsCount ? round2(totalValue - accumulated) : nominal;
