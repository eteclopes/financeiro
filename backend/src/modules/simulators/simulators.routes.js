...
// ── Simuladores são exclusivos do Plano Pro ──────────────────────────────────
router.use(requirePro);

// ── Simulador de Compras ─────────────────────────────────────────────────────
router.post('/purchase', validate(purchaseSimulationSchema), asyncHandler(async (req, res) => {
...
function validateScenarioInput(payload) {
  ...
  if (!result.success) throw new AppError('Input do cenário inválido.', 422, 'VALIDATION_ERROR', result.error.flatten().fieldErrors);
  return result.data;
}

router.post('/what-if/preview', validate(previewSchema), asyncHandler(async (req, res) => {
  const validInput = validateScenarioInput(req.body);
  const result = await whatIfService.runScenarioPreview(req.userId, req.body.monthId, req.body.type, validInput, req.body.monthsAhead);
  res.json(result);
}));

router.post('/what-if/save', validate(saveSchema), asyncHandler(async (req, res) => {
  const validInput = validateScenarioInput(req.body);
  const result = await whatIfService.saveSimulation(req.userId, req.body.monthId, { type: req.body.type, name: req.body.name, input: validInput, monthsAhead: req.body.monthsAhead });
...
router.delete('/what-if/:id', asyncHandler(async (req, res) => {
